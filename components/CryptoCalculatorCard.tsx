...
export default function CryptoCalculatorCard() {
	...
	function calculateRisk() {
		const riskMoveSize = Math.abs(
			(Number(formData.entryPrice) - Number(formData.stopLossPrice) * (1 + 0.03 / 100)) / //add a 0.03% buffer to account for slippage/spreads
			Number(formData.entryPrice),
		);

		const positionSize = (Number(formData.riskAmount) * (1 + 0.03 / 100)) / riskMoveSize;
